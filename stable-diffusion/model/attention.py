...
class SelfAttention(nn.Module):
    ...
    def forward(self, x:torch.Tensor, causal_mask=False):
        ...
        # bs, seq_length, d_embeddings -> bs, seq_length, d_embeddings * 3 -> bs, seq_length, d_embeddings
        q, k, v = self.in_proj(x).chunk(3, dim=-1)

        # bs, seq_length, d_embeddings -> bs, seq_length, H, d_embeddings / H -> bs, H, seq_length, d_embeddings / H
        ...
        # (bs, H, seq_length, seq_length) @ (bs, H, seq_len, d_embeddings / H) -> (bs, H, seq_len, d_embeddings / H)
        output = weight @ v

        # (bs, H, seq_len, d_embeddings / H) -> (bs, seq_len, H, d_embeddings / H)
